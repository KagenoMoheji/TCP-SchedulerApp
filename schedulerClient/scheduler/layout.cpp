...
void drawLayout() {
	...
	app->label = gtk_label_new("Client IP Address : ");
	...
	getOwnIPAddress();
	cout << "OwnIPAddress : " << clientIP << endl;
	app->label = gtk_label_new(clientIP);
	...
	gtk_box_pack_start(GTK_BOX(app->framehbox), app->serverEntry, FALSE, FALSE, 0);
	...
	app->connectGuide = gtk_label_new("Input the Server IP Address and a Port Number...");//エラーとか
...
void updateCalendar() {
	...
	for (int i = 0; i < DAYCNT; i++) {
		...
		if ((freeFlags[i]) && (oppoFreeFlags[i])) {
