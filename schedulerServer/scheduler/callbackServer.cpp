...
void tryConnection(GtkWidget* eventbox, gpointer data) {
	char text[CHARBUFF];
	gchar *gLabel;
	...
	//getServerIP();
	getPort();
	//cout << "Inputted Server IP Address : " << tcpServer.serverIP << endl;
	cout << "Inputted Port Number : " << port << endl;

	sprintf_s(text, "Waiting for Client...");
	gLabel = text;
	...
	//mainTCPServer(NULL);
	//スレッドの呼び出し
	TCPThread = g_thread_new("mainTCPClient", mainTCPServer, NULL);
	//スレッドの終了
...
/*
void getServerIP() {
tcpServer.serverIP = gtk_entry_get_text(GTK_ENTRY(app->serverEntry));
}
*/
